  ...
  return (
    ...
    >
      <div className="relative h-56 sm:h-72">
        ...
        />
        {story.highlight ? (
          <p className="absolute top-4 left-4 rounded-full bg-sunset-600 px-4 py-1.5 text-sm font-bold text-white shadow-md">
            {story.highlight}
          </p>
        ) : null}
      </div>
      ...
      <div className="p-6 sm:p-10">
        <h3 id={`story-${story.slug}`} className="font-display text-2xl font-bold text-lagoon-950 sm:text-3xl">
          {story.title}
        </h3>
        ...
        <p className="mt-4 rounded-xl border border-sunset-200 bg-sunset-50 px-4 py-3 text-sm leading-relaxed text-sunset-950">
          <strong className="font-semibold">
            Estimated value: {formatCentsPerPoint(cpp)} per {story.pointsUnit === "miles" ? "mile" : "point"}.
          </strong>{" "}
