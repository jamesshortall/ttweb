...
export const siteConfig = {
  ...
  cardmasterUrl: process.env.NEXT_PUBLIC_CARDMASTER_URL ?? "https://cardmaster.traveltechnician.info",
  blogUrl: process.env.NEXT_PUBLIC_BLOG_URL ?? "https://blog.traveltechnician.info",
  instagramUrl: process.env.NEXT_PUBLIC_INSTAGRAM_URL ?? "https://instagram.com/the_travel_technician",
...
export const analyticsConfig = {
  ...
  get enabled(): boolean {
    return (
      (this.provider === "plausible" || this.provider === "google") && this.id.length > 0
    );
  },
} as const;
